...
module.exports = {
    ...
    run: async(client, message, args) => {
       ...
       // if (!age) return message.channel.send(embed.setDescription("Kullanıcı için bir yaş yazılmak zorunda!"))
        ...
        message.guild.members.cache.get(member.id).setNickname(`${name} Wess`)
        db.push(`isimler_${member.id}`, ` \`${name} Wess\` (isim değiştirme>)`);
        message.channel.send(embed.setDescription(`${member} Adlı Kullanıcının İsmi \`${name} Wess\` Olarak Değiştirildi`)
